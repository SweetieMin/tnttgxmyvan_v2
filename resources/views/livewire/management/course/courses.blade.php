            ...
            <div class="theme-table">
                ...
                <div class="hidden md:block ">
                    <flux:card class="overflow-hidden border border-accent/20 rounded-xl shadow-sm">
                        <flux:table
                            container:class=" {{ $courses->hasPages() ? 'max-h-[calc(100vh-541px)]' : 'max-h-[calc(100vh-455px)]' }}"
                            ...
                            <flux:table.rows id="sortable-courses" data-academic-year-id="{{ $selectedYear ?? null }}">
                                @forelse ($courses as $course)
                                    <flux:table.row wire:key="course-desktop-{{ $course->id }}"
                                        data-id="{{ $course->id }}">
                ...
                <div class="md:hidden space-y-3" id="sortable-courses-mobile"
                    ...
                    @forelse ($courses as $course)
                        <flux:accordion wire:key="course-mobile-{{ $course->id }}" transition variant="reverse"
                            data-id="{{ $course->id }}">
                                        ...
                                        <div class="flex items-start justify-between gap-3">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex flex-col text-left">
                                                    <span
                                                        class="font-semibold text-accent-text">{{ $course->course }}</span>
                                                    <span class="text-xs text-accent-text/70">Niên khoá:
                                                        {{ $course->academicYear->name ?? 'N/A' }}</span>
                                                </div>
                                            </div>
                                            <flux:badge variant="solid" color="green">
                                                {{ $course->program->course ?? 'N/A' }}
