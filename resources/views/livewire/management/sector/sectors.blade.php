<div>
    <x-contents.layout heading="Ngành Sinh Hoạt" subheading="Quản lý danh sách và thông tin ngành sinh hoạt"
        icon="squares-plus" :breadcrumb="[['label' => 'Bảng điều khiển', 'url' => route('dashboard')], ['label' => 'Ngành Sinh Hoạt']]" buttonLabel="Thêm ngành sinh hoạt" buttonAction="addSector">

        {{-- Component Search & Filter --}}

        <div class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1">
                <x-contents.search searchPlaceholder="Tìm kiếm ngành sinh hoạt..." wire:model.live.debounce.300ms="search"
                    :years="$years" />
            </div>
        </div>
        ...
        <div x-data="{
            initSortable() {
                // Desktop sortable
                const desktopEl = document.getElementById('sortable-sectors');
 ...
 <flux:table container:class=" {{ $sectors->hasPages() ? 'max-h-[calc(100vh-541px)]' : 'max-h-[calc(100vh-455px)]' }}"
                        ...
                        <flux:table.rows id="sortable-sectors">
                            @forelse ($sectors as $sector)
                                <flux:table.row wire:key="sector-desktop-{{ $sector->id }}"
                                    data-id="{{ $sector->id }}">
                                        ...
                                        <flux:dropdown position="bottom" align="end">
                                            ...
                                            <flux:menu>
                                                <flux:menu.item class="cursor-pointer" icon="pencil-square"
                                                    wire:click='editSector({{ $sector->id }})'>
                                                    Sửa
                                                </flux:menu.item>
                                                <flux:menu.item class="cursor-pointer" icon="trash" variant="danger"
                                                    wire:click='deleteSector({{ $sector->id }})'>
                                                    Xoá
                                                </flux:menu.item>
                                            </flux:menu>
                                        </flux:dropdown>
            ...
            <div class="md:hidden space-y-3" id="sortable-sectors-mobile">
                @forelse ($sectors as $sector)
                    <flux:accordion wire:key="sector-mobile-{{ $sector->id }}" transition variant="reverse"
                        data-id="{{ $sector->id }}">
                                    ...
                                    <div class="flex items-start justify-between gap-3">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <div class="flex flex-col text-left">
                                                <span
                                                    class="font-semibold text-accent-text">{{ $sector->sector }}</span>
                                                <span class="text-xs text-accent-text/70">Niên khoá:
                                                    {{ $sector->academicYear->name ?? 'N/A' }}</span>
                                            </div>
                                        </div>
                                        <flux:badge variant="solid" color="green">
                                            {{ $sector->program->sector ?? 'N/A' }}
                                ...
                                <flux:accordion.content>
                                    <div class="space-y-3 text-sm text-accent-text/90 mt-2">
                                        <div class="pt-3 border-t border-accent/10 flex gap-2">
                                            <flux:button wire:click='editSector({{ $sector->id }})'
                                                icon="pencil-square" variant="filled" class="flex-1">
                                                Sửa
                                            </flux:button>
                                            <flux:button wire:click='deleteSector({{ $sector->id }})' icon="trash"
