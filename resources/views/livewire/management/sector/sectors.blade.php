        ...
        <div class="mt-2">
            <div x-data="{
                        ...
                        <flux:table 
                            ...
                            <flux:table.rows id="sortable-sectors">
                                @forelse ($sectors as $sector)
                                    <flux:table.row wire:key="sector-desktop-{{ $sector->id }}" data-id="{{ $sector->id }}">
                                            ...
                                            <flux:dropdown position="bottom" align="end">
                                                ...
                                                <flux:menu>
                                                    ...
                                                    <flux:menu.item class="cursor-pointer" icon="trash"
                                                        variant="danger" wire:click='deleteSector({{ $sector->id }})'>
                                                        Xoá
                                                    </flux:menu.item>
                                                </flux:menu>
                                            </flux:dropdown>
                ...
                <div class="md:hidden space-y-3" id="sortable-sectors-mobile">
                    @forelse ($sectors as $sector)
                        <flux:accordion wire:key="sector-mobile-{{ $sector->id }}" transition variant="reverse" data-id="{{ $sector->id }}">
                                        ...
                                        <div class="flex items-start justify-between gap-3">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex flex-col text-left">
                                                    <span class="font-semibold text-accent-text">{{ $sector->sector }}</span>
                                                    <span class="text-xs text-accent-text/70">Niên khoá: {{ $sector->academicYear->name ?? 'N/A' }}</span>
                                                </div>
                                            </div>
                                            <flux:badge variant="solid" color="green">
                                                {{ $sector->program->sector ?? 'N/A' }}
