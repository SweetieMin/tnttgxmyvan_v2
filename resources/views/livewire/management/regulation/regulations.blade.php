<div>
    <x-contents.layout heading="Nội quy" subheading="Quản lý danh sách và thông tin nội quy" icon="squares-plus"
        ...
        <div class="flex flex-col sm:flex-row gap-4">
            <div class="flex-1">
                <x-contents.search searchPlaceholder="Tìm kiếm Nội quy / điểm..." :years="$years" />
            </div>
        </div>
        ...
        <div x-data="{
            ...
            <div class="hidden md:block ">
                <flux:card class="overflow-hidden border border-accent/20 rounded-xl shadow-sm">
                    <flux:table container:class="max-h-[calc(65vh-90px)] overflow-y-auto custom-scrollbar"
                        ...
                        <flux:table.rows id="sortable-regulations" data-academic-year-id="{{ $selectedYear ?? null }}">
                            @forelse ($regulations as $regulation)
                                <flux:table.row wire:key="regulation-desktop-{{ $regulation->id }}"
            ...
            {{-- Mobile Card View --}}
            <div class="md:hidden space-y-3" id="sortable-regulations-mobile"
                data-academic-year-id="{{ $selectedYear ?? null }}">
                @forelse ($regulations as $regulation)
                    <flux:accordion wire:key="regulation-mobile-{{ $regulation->id }}" transition variant="reverse"
                        data-id="{{ $regulation->id }}">
                                    ...
                                    <div class="flex items-center justify-between gap-2">
                                        <div class="flex items-center gap-2 flex-1">
                                            ...
                                            <div class="flex flex-col text-left">
                                                <span
                                                    class="font-semibold text-accent-text">{{ $regulation->name }}</span>
                                                <span
                                                    class="text-sm text-accent-text/80">{{ $regulation->description }}</span>
                                            </div>
                                        </div>
                                ...
                                <flux:accordion.content>
                                    <div class="space-y-3 text-sm text-accent-text/90 mt-2">
                                        <div class="pt-3 border-t border-accent/10 flex gap-2">
                                            ...
                                            <flux:button wire:click='deleteRegulation({{ $regulation->id }})'
